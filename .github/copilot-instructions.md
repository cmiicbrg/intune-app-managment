...
- **#9 migration state**: every wrapper is native (`Invoke-MgGraphRequest` against `beta`, `-OutputType PSObject`) including `Publish-InteropWin32App` (create app → content version → extract the pre-encrypted payload from the `.intunewin` → chunked Azure Storage upload with SAS renewal → commit with `fileEncryptionInfo` from Detection.xml → PATCH `committedContentVersion`). `updateRelationships` REPLACES an app's full relationship set, so the supersedence/dependency wrappers read and re-submit the other relationship type. The module is no longer called at runtime; the bootstrap and auth shim go away in the final #9 cleanup.
