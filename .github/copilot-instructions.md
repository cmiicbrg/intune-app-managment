...
- **App families**: `Get-AppFamilyCatalog` / `Resolve-AppFamily` / `Get-AppFamilyBaseName` / `Get-IntuneAppVersion` in `SharedFunctions.ps1` are the one way to map an Intune app to its AppConfig family and version. Family base name = the template text before `{0}` (equivalently, the display name before its first version number — a test pins the equivalence). Deploy-ToIntune.ps1 and the inventory/cleanup tooling must use these rather than re-deriving names.
